...
class _EdgeDBRoleQuery:
        ...
        if self.in_memory:
            if role_id and permission_key:
                ...
                return {
                    ...
                    for role_id, permissions in self.roles.items()
    ...
    def validate(self, role_id, permission_key, permission_val):
        ...
        if self.in_memory:
            return (
                self.roles.get(role_id, {}).get(permission_key, None) == permission_val
            )
        else:
            self.cursor.execute(
                "SELECT permissions FROM roles WHERE role_id = ?", (role_id,)
...
class AuthLiteClient:
    ...
    def __init__(
        ...
        self.api_key = api_key
        self.org_id = org_id
        self.signed_key = self.jwt_encode(
            key=self.secret_key, data={"api_key": self.api_key}
        )
        self.API_BASE_URL = API_BASE_URL
    ...
    def re_auth(self, code):
        ...
        params = {"code": code, "api_key": self.api_key, "signed_key": self.signed_key}
        headers = {"accept": "application/json"}
        response = requests.get(url, headers=headers, params=params)
